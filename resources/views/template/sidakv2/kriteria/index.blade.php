...
<div class="content">
	...
	<div class="box box-solid box-primary">
     ...
     @include('template/sidakv2/kriteria.add')
     @include('template/sidakv2/kriteria.export')
<script type="text/javascript">
 ...
 $(document).ready(function() {
    ...
    $("#ExportButton").click(function() {
    	 ...
    	 $.ajax({
            url: BASE_URL+ `/api/kriteria?page=${page}&per_page=all`,
            method: 'GET',
    ...
    $('#search-input').keyup( () => {
 		 ...
 		 { 	
             ...
             row +=`<tr><td colspan="8" align="center"> <b>Loading ...</b></td></tr>`;
              content.append(row);
	         $.ajax({
	            url: BASE_URL + `/api/kriteria/search?page=${page}&per_page=${itemsPerPage}`,
	            data:{'search':search},
	            method: 'POST',
	            success: function(response) {
	            	list = response.data;
    ...
    function deleteItems(ids) {
        ...
        $.ajax({
            url:  BASE_URL +`/api/kriteria/selected`,
            method: 'POST',
            data: { data: ids },
            success: function(response) {
              ...
              options.forEach(function(opt, arr) 
              {
               ...
               row +=`<td>`; 
                row +=`<div class="btn-group list-menu-table">`;
                 ...
                 row +=`<div id="Detail" data-param_id="`+ item.id +`" data-toggle="modal" data-target="#modal-edit-${item.id}" type="button" data-toggle="tooltip" data-placement="top" title="Detail Data"  class="pointer btn-padding-action pull-left"><i class="fa-icon icon-detail" ></i></div>`;
            ...
            row +=`<div class="modal-dialog">`;
                row +=`<div class="modal-content">`;
				       ...
				       row +=`<div class="modal-header">`;
				         ...
				         row +=`<h4 class="modal-title">Detail Kriteria</h4>`;
				       row +=`</div>`;
				       ...
				       row +=`<form   id="FormSubmit-`+ item.id +`">`;
					        row +=`<div class="modal-body">`;
				                 ...
				                 row +=`<div id="category-alert-`+ item.id +`" class="form-group has-feedback" >`;
				                  ...
				                  row +=`<input readonly type="text" class="form-control" name="category" placeholder="Kategori" value="`+ item.category +`">`;
				                 ...
				                 row +=`</div>`;
				                   ...
				                   row +=`<div id="description-alert" class="form-group has-feedback" >`;
						              ...
						              row +=`<textarea readonly class="form-control textarea-fixed" placeholder="Keterangan" name="description">`+ item.description +`</textarea>`;
        ...
        $( "#content" ).on( "click", "#Edit", (e) => {
            ...
            row +=`<div class="modal-dialog">`;
                row +=`<div class="modal-content">`;
				       ...
				       row +=`<div class="modal-header">`;
				         row +=`<button type="button" class="close" data-dismiss="modal">&times;</button>`;
				         row +=`<h4 class="modal-title">Edit Kriteria</h4>`;
				       row +=`</div>`;
   ...
   function BtnTableDelete(item){
    ...
    function ExportExel()
    {
	  ...
	  XLSX.writeFile(wb, "Repot-data-kriteria-"+ time +".xlsx");
